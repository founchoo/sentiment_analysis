...
> to repository.
>
> Please do follow steps from 2 to 4 to ensure your `.csv` files are ready to be processed.
> The `.json` files are generated by program, those files will be generated when you finish traning your model.

1. Clone this repository.
2. Create `data` folder in root directory of your cloned project.
3. Download datasets from [here](https://github.com/founchoo/sentiment_analysis?tab=readme-ov-file#datasets-from-kaggle) and put them in `data` folder.
4. Rename those two downloaded files to `normal_offensive_data.csv` and `real_fake_data.csv` respectively.
5. Run `flutter run` in the root directory.
